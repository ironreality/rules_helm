...
cat $(location @com_github_ironreality_rules_helm//:runfiles_bash) >> $@
...
export HELM=\$$(rlocation com_github_ironreality_rules_helm/helm)
...
def helm_chart(name, srcs, update_deps = False):
    ...
    native.genrule(
        ...
        tools = ["@com_github_ironreality_rules_helm//:helm"],
        cmd = """
...
$(location @com_github_ironreality_rules_helm//:helm) package {package_flags} $$CHARTLOC
mv *tgz $@
""".format(
...
def _helm_cmd(cmd, args, name, helm_cmd_name, values_yaml = None, values = None):
    binary_data = ["@com_github_ironreality_rules_helm//:helm"]
    if values_yaml:
...
def helm_release(name, release_name, chart, values_yaml = None, values = None, namespace = ""):
    ...
    genrule_srcs = ["@com_github_ironreality_rules_helm//:runfiles_bash", chart]
